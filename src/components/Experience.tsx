  ...
  return (
    <section className="container-responsive py-20" id="experience">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">Highlights from recent roles with impact and ownership.</p>
      </div>
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-12 relative">
        ...
        {items.map((r, idx) => (
          <motion.div
            ...
            className={`relative ${idx % 2 === 0 ? 'md:pr-14' : 'md:pl-14 md:col-start-2'}`}
            ...
            <div className="card p-6 md:p-7">
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="shrink-0">
                  <span className="inline-block text-xs px-3 py-1 rounded-full bg-brand/10 text-brand border border-brand/20">{r.period}</span>
                </div>
              </div>
              {/* Divider */}
              ...
              {/* Optional website */}
              {r.website && (
                <a className="mt-3 inline-block text-sm text-brand hover:underline" href={r.website.startsWith('http') ? r.website : `https://${r.website}`} target="_blank" rel="noreferrer">Visit website ↗</a>
              )}
            </div>
          </motion.div>
        ))}
